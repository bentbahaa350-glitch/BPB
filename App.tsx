...
{/* Close Button (only if not initial setup) */}
{messages.length > 0 && (
<button
...
<p className="text-[10px] text-slate-500 dark:text-slate-500 mt-1 uppercase tracking-widest font-bold">Hypertrophy AI System</p>
</div>
</div>
...
const isSectionGenerating = isGeneratingImages.has(sectionId);
...
<Sparkles size={16} className={isSectionGenerating ? 'animate-pulse' : ''}/>
{isSectionGenerating ? 'جاري توليد دليل الحركات...' : 'دليل الحركات التوضيحية'}
</h3>
{isSectionGenerating && <Loader2 size={16} className="animate-spin text-emerald-600 dark:text-emerald-500" />}
</div>
...
<div className="w-12 h-12 rounded-2xl bg-slate-200 dark:bg-slate-700/30 mb-4 animate-pulse" />
<span className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">{ex}</span>
</div>
)}
</div>
))}
...
{msg.role === 'assistant' ? 'System Program v2.1' : 'Athlete Input'}
</div>
</div>
))}
{isLoading && (
...
<span className="text-sm font-black text-emerald-600 dark:text-emerald-400 tracking-[0.2em] uppercase">Processing Hypertrophy Data...</span>
</div>
</div>
</div>
</div>
